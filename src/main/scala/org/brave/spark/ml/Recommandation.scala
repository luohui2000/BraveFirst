...
/*    val randomNumber1 = scala.util.Random.nextInt().%(234934)
    ...
    hc.sql("cache table ratings_training")
    hc.sql("cache table movies")
    //    val allUsers = hc.sql("select distinct(userid) from ratings")
    if (hc.sql(s"select userid from ratings_training where userid=$inputUserId").count() == 0) {
...
//   randomRecommandMovies.foreach { println }
    ...
    //模型的保存和加载
    model.save(sc, s"model/ALS_$today")
    //    val safedModel = MatrixFactorizationModel.load(sc, s"model/ALS_$today")
  }
}